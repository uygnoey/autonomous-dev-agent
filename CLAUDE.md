...
3. **QA Gate 통과 → QC 테스트 통과 → 전체 기능 100% 완성**까지 무한 반복
4. QA(예방): 설계 검증 → 정적 분석/패턴 준수 → 스모크 테스트. Gate BLOCK 시 수정 후 재검증
5. QC(검출): QA Gate 통과 후 대량 테스트 실행 (모듈 1만건, E2E 10만건). 실패 시 수정 후 재실행
6. 코드 완성 후 → **documenter 에이전트가 전체 문서 생성**
7. 토큰 한도 도달 시 → 리셋될 때까지 대기 후 이어서 진행
8. 크리티컬 이슈만 즉시 사람에게 질문, 나머지는 완성 후 모아서 전달
...
- **qa**: 품질 보증(예방) — 코딩 전 스펙/설계 검증, 코딩 중 정적 분석/패턴 준수, QC 진입 전 스모크 테스트
- **qc**: 품질 검증(검출) — 대량 테스트 케이스 자동 생성 및 실행 (모듈 1만건, E2E 10만건)
